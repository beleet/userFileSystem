...
ufs_open(const char *filename, int flags) {
    ...
    if (ptr != NULL) (ptr->refs)++;
        ...
        ptr->block_list = NULL, ptr->current_block = NULL, ptr->name = strdup(filename), ptr->next = NULL, ptr->prev = NULL;
...
ufs_write(int fdes, const char *buffer, size_t size) {
    ...
    while (bytes < size) {
        ...
        fd->position += written, block_offset += written, block->occupied = block->occupied > block_offset ? block->occupied : block_offset;
